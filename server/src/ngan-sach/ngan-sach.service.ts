...
export class NganSachService {
  ...
  async findAll(
    ...
    const where: FindOptionsWhere<NganSach> = { nguoiDungId };
  ...
  async findOne(id: number, nguoiDungId: number): Promise<NganSach> {
    const nganSach = await this.nganSachRepo.findOne({
  ...
  async update(
    ...
    const changed =
      newThang !== nganSach.thang ||
      newNam !== nganSach.nam ||
      newDmId !== nganSach.danhMucId;
    if (changed) {
      ...
      if (conflict && conflict.id !== id) {
        throw new ConflictException(
          `Đã tồn tại ngân sách cho tháng ${newThang}/${newNam}`,
        );
      }
    }
  ...
  async remove(id: number, nguoiDungId: number): Promise<{ message: string }> {
    ...
    return {
      message: `Đã xóa ngân sách tháng ${nganSach.thang}/${nganSach.nam}`,
    };
  }
}
